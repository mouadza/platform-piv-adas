...
from django.db import transaction
from admin_config.models import Projet, Affectation, Role, CustomUser
from django.db import transaction
from admin_config.models import Projet, Affectation, Role, CustomUser, Vehicule
...
@transaction.atomic
def update_project_service(projet_id, data):
    ...
    for aff in affectations_data:
        user = CustomUser.objects.get(id=aff["user"])
        role = Role.objects.get(code=aff["role"])
        Affectation.objects.create(user=user, role=role, projet=projet)
