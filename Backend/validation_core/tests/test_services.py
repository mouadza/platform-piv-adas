...
from validation_core.services import access_control
from validation_core.services.audit_service import (
    get_client_ip,
    get_user_agent,
    log_audit_event,
    sanitize_metadata,
)
from validation_core.services.dashboard_service import admin_dashboard_service
...
from validation_core.services.gamme_validation_dates import (
...
class OTPServiceTests(TestCase):
    ...
    def test_bootstrap_admin_promotes_existing_user(self):
        user = CustomUser.objects.create_user(
            username="Existing Bootstrap",
            email="ta45177@stellantis.com",
            password="Temporary123",
            is_active=False,
        )

        promoted_user = get_or_create_bootstrap_admin("ta45177@stellantis.com")
        promoted_user.refresh_from_db()

        self.assertEqual(promoted_user.id, user.id)
        self.assertTrue(promoted_user.is_active)
        self.assertTrue(promoted_user.is_staff)
        self.assertTrue(promoted_user.is_superuser)
        self.assertFalse(promoted_user.has_usable_password())

    def test_bootstrap_admin_can_request_and_verify_otp(self):
        with patch(
            "validation_core.services.otp_service.generate_non_repeated_code",
            return_value=("123456", hash_otp_code("123456")),
        ), patch("validation_core.services.otp_service.send_otp_email"):
            response = request_email_otp("ta45177@stellantis.com")

        user = CustomUser.objects.get(email="ta45177@stellantis.com")
        ...
        tokens = verify_email_otp(user.email, "123456")
    ...
    def test_generate_non_repeated_code_skips_recent_hashes(self):
        ...
        with patch("validation_core.services.otp_service.secrets.randbelow") as rand:
    ...
    @override_settings(CELERY_LOGIN_EMAIL_ASYNC=True)
    def test_request_email_otp_queues_email_job_when_async_enabled(self):
        with patch(
            "validation_core.services.otp_service.generate_non_repeated_code",
            return_value=("123456", hash_otp_code("123456")),
        ), patch(
            ...
            apply_async.return_value.id = "task-login-otp"

            response = request_email_otp(self.user.email)

        self.assertEqual(response["message"], GENERIC_OTP_RESPONSE)
        self.assertEqual(EmailOTP.objects.filter(user=self.user).count(), 1)
        job = EmailJob.objects.get(email_type=EmailJob.Type.LOGIN_OTP)
        self.assertEqual(job.recipient_email, self.user.email)
        self.assertEqual(job.status, EmailJob.Status.PENDING)
        self.assertEqual(job.celery_task_id, "task-login-otp")
        self.assertIn("123456", job.body_text)
        apply_async.assert_called_once_with(args=[job.id], queue="email_high")
...
class TokenSerializerTests(TestCase):
    ...
    def test_password_login_serializer_is_disabled(self):
    ...
    def test_superuser_token_contains_admin_claims(self):
        user = CustomUser.objects.create_superuser(
            username="Root Claims",
            email="root.claims@stellantis.com",
            password="StrongPassword123",
        )
        serializer = CustomTokenObtainPairSerializer()
