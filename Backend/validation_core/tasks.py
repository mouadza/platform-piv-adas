...
from validation_core.models import BackgroundJob, CustomUser, GammeParsedData, Projet
from validation_core.services.email_service import send_account_authorized_email
from validation_core.services.email_jobs import deliver_email_job
from validation_core.services.gamme_parse_storage import parse_and_store_gamme
from validation_core.services.project_kpi_excel_service import (
...
@shared_task(
    bind=True,
    name="admin_config.send_email_job",
    max_retries=2,
    default_retry_delay=30,
)
def send_email_job_task(self, email_job_id):
    try:
        return deliver_email_job(email_job_id)
...
def dispatch_account_authorized_email(user):
    try:
        result = send_account_authorized_email_task.apply_async(
            args=[user.id],
