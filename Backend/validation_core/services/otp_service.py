...
from validation_core.models import EmailJob, EmailOTP
from validation_core.services.email_jobs import enqueue_email_job, send_email_message
...
def get_or_create_bootstrap_admin(email):
    ...
    is_bootstrap_email = is_bootstrap_admin_email(email)
...
def build_otp_email_payload(user, code):
...
def send_otp_email(user, code):
    payload = build_otp_email_payload(user, code)
    send_email_message(
...
def dispatch_otp_email(user, code):
    payload = build_otp_email_payload(user, code)

    if not getattr(settings, "CELERY_LOGIN_EMAIL_ASYNC", False):
        send_otp_email(user, code)
        return {"queued": False, "email_job_id": None}

    job = enqueue_email_job(
        email_type=EmailJob.Type.LOGIN_OTP,
        recipient_email=payload["recipient_email"],
        subject=payload["subject"],
        body_text=payload["body_text"],
        body_html=payload["body_html"],
        metadata=payload["metadata"],
        queue=getattr(settings, "CELERY_LOGIN_EMAIL_QUEUE", "email_high"),
        async_enabled=True,
        fallback_sync=getattr(settings, "CELERY_LOGIN_EMAIL_FALLBACK_SYNC", True),
    )

    return {"queued": job.status != EmailJob.Status.SENT, "email_job_id": job.id}
...
def request_email_otp(email):
    ...
    if not user:
        user = get_or_create_bootstrap_admin(email)
