...
from validation_core.services.gamme_parse_storage import get_ready_parsed_gamme
...
def sync_gamme_validation_dates(gamme, overwrite=True, notify=False, actor=None):
    old_date_debut = gamme.date_debut
    old_date_fin = gamme.date_fin
    ...
    if gamme.date_debut != date_debut or gamme.date_fin != date_fin:
    ...
    if notify:
        ...
        if gamme.date_debut and not old_date_debut:
            notify_admins_gamme_started(gamme, actor=actor)
        elif gamme.date_debut:
            notify_admins_gamme_started(gamme, actor=actor)

        if gamme.date_fin and not old_date_fin:
            notify_admins_gamme_finished(gamme, actor=actor)
        elif gamme.date_fin:
            notify_admins_gamme_finished(gamme, actor=actor)
