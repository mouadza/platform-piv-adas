...
from validation_core.models.gamme import Gamme, GlobalEVComment
from validation_core.models.results import StepValidation
from validation_core.serializers.gamme_serializers import GlobalEVCommentSerializer
...
from validation_core.services.notification_service import notify_admins_gamme_started
...
class StepValidationCreateView(APIView):
    ...
    def post(self, request):
        ...
        had_previous_validations = False
        ...
        if gamme_id:
            ...
            had_previous_validations = StepValidation.objects.filter(
                gamme_id=gamme.id
            ).exists()
        ...
        if serializer.is_valid():
            ...
            if not had_previous_validations:
                notify_admins_gamme_started(validation.gamme, actor=request.user)

            sync_gamme_validation_dates(
                validation.gamme,
                notify=True,
                actor=request.user,
            )
